      ...
      <div className="py-12 w-[90%] md:w-full mx-auto">
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div className="bg-white overflow-hidden shadow-sm rounded-lg text-center md:text-left">
            <h1 className="p-4 text-[20px]">Bienvenido, {auth?.user?.name}</h1>
            {user?.role === 'admin' && (
            ...
            {registered_users_last_week?.length && user?.role === 'admin' ? (
              <h2 className="p-4 text-[16px]">
                ...
                {registered_users_last_week.length > 1 ? ' han ' : ' ha '}
                registrado{' '}
            ...
            {user?.role === 'admin' && (
              <div className="bg-white overflow-hidden shadow-sm rounded-lg mt-4">
                <div className="p-6 bg-white border-b border-gray-200">
                  <h2 className="text-[20px]">Mensajes</h2>
                  <div className="mt-4">
                    <CreateMessage />
                  </div>
                </div>
          ...
          <div>
            <div className="bg-white overflow-hidden shadow-sm rounded-lg mt-4">
              <div className="p-6 bg-white border-b border-gray-200">
                <h2 className="text-[20px]">Exams Available</h2>
                <div className="mt-4 grid grid-cols-3 gap-4">
                  {props.exams.map((exam, index) => (
                    <div key={index} className="border p-4 rounded">
                      <InertiaLink href={`/exam/${exam.id}`}>
                        <h3>{exam.name}</h3>
